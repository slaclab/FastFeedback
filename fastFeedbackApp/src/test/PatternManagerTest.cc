...
/*
void FF::PatternManagerTest::testGetPatterns() {
    PatternManager pm;
    ...
    const PatternMask *nullPattern = NULL;

    CPPUNIT_ASSERT_EQUAL(4, pm.getNumPatterns());
    CPPUNIT_ASSERT(nullPattern == pm.getPatternMask(0));
    CPPUNIT_ASSERT(nullPattern == pm.getPatternMask(5));

    const PatternMask *pt1 = pm.getPatternMask(1);
    CPPUNIT_ASSERT(pt1 != nullPattern);
    CPPUNIT_ASSERT(p1 == *pt1);

    const PatternMask *pt2 = pm.getPatternMask(2);
    CPPUNIT_ASSERT(pt2 != nullPattern);
    CPPUNIT_ASSERT(p2 == *pt2);

    const PatternMask *pt3 = pm.getPatternMask(3);
    CPPUNIT_ASSERT(pt3 != nullPattern);
    CPPUNIT_ASSERT(p3 == *pt3);

    const PatternMask *pt4 = pm.getPatternMask(4);
    CPPUNIT_ASSERT(pt4 != nullPattern);
    CPPUNIT_ASSERT(p4 == *pt4);
}

void FF::PatternManagerTest::testGetPatternIndex() {
    PatternManager pm;
    ...
    CPPUNIT_ASSERT_EQUAL(1, pm.getPatternMaskIndex(p1));
    CPPUNIT_ASSERT_EQUAL(2, pm.getPatternMaskIndex(p2));
    CPPUNIT_ASSERT_EQUAL(3, pm.getPatternMaskIndex(p3));
    CPPUNIT_ASSERT_EQUAL(-1, pm.getPatternMaskIndex(p4));
}
*/