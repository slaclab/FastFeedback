 ...
 * @author L.Piccoli
 */
TrajectoryFitGeneral::TrajectoryFitGeneral(std::string algorithmName) :
...
_fitPrepStats(50, "TrajectoryFit2 fit::pre"),
_fitGelsStats(50, "TrajectoryFit2 fit::gels"),
_fitPostStats(50, "TrajectoryFit2 fit::post"),
...
TrajectoryFitGeneral::~TrajectoryFitGeneral() {
}
 ...
 * @author L.Piccoli
 */
int TrajectoryFitGeneral::reallyConfig() {
...
/**
 * This method is invoked when the feedback mode is switched between 
 * COMPUTE to ENABLE. The F and G Matrices are reloaded.
 */
int TrajectoryFitGeneral::reset() {
...
int TrajectoryFitGeneral::calculate() throw (Exception) {
  checkConfig();
    ...
    if (numBpms < 0) {
      throw Exception("Invalid number of Measurement Devices");
    }

    _getMeasStats.end();
    if (fit(_fitX, _fitY, _calcStates) != 0) {
      throw Exception("Fail in fit function");
    }

    //    std::cout << _calcStates << std::endl;
    ...
    if (getCorrectorKicks(_calcStates, _setpoint, _actuatorDelta) != 0) {
      throw Exception("Fail in getCorrectorKicks()");
    }

    // TODO: It TrajectoryFit2 is used by the LTU feedback, replace
    ...
    // actuatorDelta
    updateStates(_calcStates);

    updateActuators(_actuatorDelta);
 ...
 * @author L.Piccoli
 */
void TrajectoryFitGeneral::convertActuatorBdes(Vector& actuatorDelta) {
 ...
 */
int TrajectoryFitGeneral::getMeasurements() {
    if (_measurements == NULL) {
      throw Exception("TrajectoryFit: _measurements vector is NULL");
    }
    ...
    if (getFailed) {
      ...
      //                << Log::flushpv;
	throw Exception("Cannot read sensors, no change");
        return -1;
    }
 ...
 * @author L.Piccoli
 */
int TrajectoryFitGeneral::fit(Vector &fitX, // Xsf
        Vector &fitY, // YsF
        Vector &states) { // p
    ...
    _loopConfiguration->_logger << Log::flagAlgo << Log::dpInfo
				<< "INFO: TrajectoryFit::fit() M1="
				<< states[0] << Log::dp;
    ...
    if (states.size() < std::max(_fitMatrixGELS.size1(), _fitMatrixGELS.size2())) {
      _loopConfiguration->_logger << Log::flagAlgo << Log::dpError
				  << "ERROR: TrajectoryFit::fit() state size is wrong state="
				  << (int)states.size() << " fitMatrix("
				  << (int)_fitMatrixGELS.size1() << ", "
				  << (int)_fitMatrixGELS.size2() << ") - PLEASE CHECK CONFIGURATION" << Log::dp;
      throw Exception(_loopConfiguration->_logger.getLastMessage());
    }
    ...
    if (status != 0) {
      _loopConfiguration->_logger << Log::flagAlgo << Log::dpError
				  << "ERROR: TrajectoryFit::fit() gels returned "
				  << (int)status << Log::dp;
      throw Exception(_loopConfiguration->_logger.getLastMessage());
    }
    ...
    // Make sure no states are NANs.
    for (int i = 0; i < (int) states.size(); ++i) {
      if (std::isnan(states[i])) {
	_loopConfiguration->_logger << Log::flagAlgo << Log::dpInfo
				    << "ERROR: TrajectoryFit::fit() S" << i << "="
				    << states[i] << Log::dp;
 ...
 * @author L.Piccoli
 */
int TrajectoryFitGeneral::getCorrectorKicks(Vector &states, Vector &setpoints,
        Vector &theta) {
    ...
    _loopConfiguration->_logger << Log::flagAlgo << Log::dpInfo
				<< "INFO: TrajectoryFit::getCorrKicks() Delta 1="
				<< theta[0] << Log::dp;
...
int TrajectoryFitGeneral::updateActuators(Vector& actuatorDelta) throw (Exception) {
    ...
    for (ActuatorSet::iterator it = _actuators->begin();
	  ...
	  if (actuatorLimit) _exceptionMessage += ",";
	  _exceptionMessage += " ";
...
int TrajectoryFitGeneral::updateStates(Vector &states) throw (Exception) {
  //    std::cout << &_states << std::endl;
    ...
    for (StateSet::iterator it = _states->begin();
	  ...
	  if (stateLimit) _exceptionMessage += ",";
	  _exceptionMessage += " ";
	  _exceptionMessage += (*it)->getDeviceName();

	  std::ostringstream strstream;
	  strstream << stateValue;
	  _exceptionMessage += " (";
 ...
 * @author L.Piccoli
 */
void TrajectoryFitGeneral::show() {
}
 ...
 * @author L.Piccoli
 */
void TrajectoryFitGeneral::showDebug() {
    _getMeasStats.show("    ");
    _fitPrepStats.show("    ");
    _fitGelsStats.show("    ");
    _fitPostStats.show("    ");
